    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (result.download === true) {
                ...
                entry["har-fetch-download-start-date-time"] =
...
module.exports.xhrMock = function(entry, result, dest) {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            const responseTypeHeader = (headers || []).find(
                header => header && header.name && (header.name.toLowerCase() === "content-type")
            );
            let responseType = responseTypeHeader && responseTypeHeader.value;
            if (!responseType) {
                const buffer = readchunk.sync(path, 0, fileType.minimumBytes);
                responseType =
                    istextorbinary.isTextSync(path, buffer) && "text/plain";
            }
            ...
            const _headerstr = (headers || [])
                .filter(header => header.name && header.name.charAt(0) !== ":")
                .map((header, index) => {
            ...
            let _bodystr;
            if (isText) {
                _bodystr = !isjson ? `'${str}'` : `JSON.stringify(${str})`;
            } else {
                let base64str = encodeURIComponent(str);
                base64str = new Buffer(base64str).toString("base64");
                _bodystr = `decodeURIComponent(window.atob("${base64str}"))`;
            }
...
module.exports.extract = function(entry, dest) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const {
                headers = [], content, status, statusText
            } =
            response || {};
            ...
            if (!response ||
                ...
                debugwarn(
                    "%s response invalid status %d(%s) , try to download it again",
                    entry.request.url,
                    status,
                    statusText
                );
                downloadTo(entry, dest)
                    .then(resolve)
                    .catch(err => {
            ...
            const isText = charset !== false;
            if (isText) {
                buf = Buffer.from(text, encoding || charset);
            } else {
                buf = Buffer.from(text, encoding);
            }
            const stream = new PassThrough();
            const _dest = createWriteStream(dest, {
                encoding: charset === false ? undefined : charset,
                autoClose: true
            });
            stream.on("error", reject);
            _dest.on("error", reject);
        ...
        fetch(url, {
            ...
            .then(response => {
                ...
                if (status < 200 || status >= 400) {
                    const err = new Error(
                        `${statusText ||
                            ""} download ${method}:${url} faild,with header:${JSON.stringify(
                            _headers
                        )} and body:${JSON.stringify(text)}`
                    );
